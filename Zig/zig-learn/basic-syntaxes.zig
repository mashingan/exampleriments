...
test "set enum ordinal value" {
    ...
    try expect(@enumToInt(Value2.thousand) == 1_000);
...
const Tagged = union(Tag) { a: u8, b: f16, c: bool };
...
const TaggedInfer = union(enum) { a: u8, b: f16, c: bool };
const TaggedInfer2 = union(enum) { a: u8, b: f16, c: void };
...
const hex_floating_float: f64 = 0x103.70p-5;
const another_hex_float: f64 = 0x103.70;
const yet_another_hex_float: f64 = 0x103.70P-5;
const lightspeed: f64 = 299_792_458.000_000;
const nanosecond: f64 = 0.000_000_001;
const more_hex: f64 = 0x1234_5678.9ABC_DEFp-10;
...
test "optionals" {
    ...
    const data = [_]i32{ 1, 2, 3, 4, 5, 7, 8, 12 };
    for (data) |v, i| {
        if (v == 10) {
            found_index = i;
            break;
...
test "optional-if" {
    var ent_num: error{UnknownEntity}!i32 = 5;
    if (ent_num) |entity| {
        try expect(@TypeOf(entity) == i32);
        try expect(entity == 5);
    } else |err| {
        _ = err catch {};
        unreachable;
...
fn show_window2(_: *Window2) void {}
...
test "string literal" {
    //try expect(@TypeOf("hello 異世界") == *const [12:0]u8);
...
const memlen = @import("std").mem.len;
...
test "vector looping" {
    ...
    var sum = blk: {
        var tmp: u10 = 0;
        var i: u8 = 0;
        while (i < memlen(x)) : (i += 1) tmp += x[i];
        break :blk tmp;
    };
    try expect(sum == 510);
}

const varr: [4]f32 = @Vector(4, f32){ 1, 2, 3, 4 };
